...
import { createMssqlStorage } from '../src/index.js';

/**
 * Full-stack proof: the idp-core storage seam and this MSSQL adapter,
 * together, can back a real initIdentityProvider() call and drive an actual
 * signup -> verify -> login -> refresh -> logout HTTP round trip through
 * idp-core's own buildRouter() — the same shape as the Postgres/DynamoDB
 * adapters' smoke tests. Unlike the DynamoDB adapter (dynalite's
 * TransactWriteItems gap), this runs against real SQL Server, so
 * createSessionForLogin's atomicity is exercised for real here too.
 */
...
before(async () => {
    ...
    state = await initIdentityProvider({
        ...
        storage: {
            factory: (resolvedConfig, deps) => createMssqlStorage({ pool: db.poolConfig }, deps),
        },
...
after(async () => {
    ...
    // createMssqlStorage() builds its OWN pool (separate from db.pool, which
    // build-test-db.js uses for migrations/truncation) — close it before
    // dropping the underlying per-call database.
    await state.storage.close();
    await db.stop();
});

test('signup -> verify -> login -> refresh (rotation) -> logout, backed entirely by the MSSQL adapter', async () => {
    ...
    assert.ok(verificationCapture[email], 'onVerificationEmailRequested fired with a real code from the MSSQL-backed verification-token table');
