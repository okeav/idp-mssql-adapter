...
import { MssqlServiceKeyRepository } from '../src/repositories/mssql-service-key.repository.js';

let db, repo;

before(async () => {
    db = await buildTestDb();
    repo = new MssqlServiceKeyRepository(db.pool);
});
after(async () => db.stop());
...
test('listPublishable returns only ACTIVE/ROTATING keys', async () => {
    ...
    await db.pool.request().input('kid', sql.NVarChar, 'k2').query("UPDATE dbo.idp_service_keys SET status = 'REVOKED' WHERE kid = @kid");
