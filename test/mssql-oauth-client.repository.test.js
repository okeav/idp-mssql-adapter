...
import { MssqlOAuthClientRepository } from '../src/repositories/mssql-oauth-client.repository.js';

let db, repo;

before(async () => {
    db = await buildTestDb();
    repo = new MssqlOAuthClientRepository(db.pool);
});
after(async () => db.stop());
...
test('updateByClientId patches only provided fields (including a numeric TTL) and never leaks the secret', async () => {
    await repo.create(sampleInput());
    const updated = await repo.updateByClientId('client-abc', { name: 'Renamed Client', status: 'ACTIVE', accessTokenTTL: 7200 });
    assert.equal(updated.name, 'Renamed Client');
    assert.equal(updated.status, 'ACTIVE');
    assert.equal(updated.accessTokenTTL, 7200);
