...
before(async () => {
    ...
    repo = new MssqlUserRepository(db.pool, { hashEmail, normalizeEmail });
});
after(async () => db.stop());
...
test('create assembles profile + externalProviders correctly, and normalizes/hashes the email', async () => {
    ...
    const { recordset: [row] } = await db.pool.request()
        ...
        .query('SELECT email_hash FROM dbo.idp_users WHERE id = @id');
    assert.equal(row.email_hash, hashEmail('alice@example.com'));
});

test('create returns a lowercase GUID id, matching crypto.randomUUID() casing', async () => {
    const user = await repo.create({ email: 'guid-case@example.com' });
    assert.equal(user.id, user.id.toLowerCase());
});
...
test('updateById with a lockUntil Date and lockUntil: null (BIT/DATETIME2 type-binding sanity)', async () => {
    const created = await repo.create({ email: 'lock@example.com' });
    const locked = await repo.updateById(created.id, { lockUntil: new Date(Date.now() + 60_000), failedLoginAttempts: 5 });
    assert.ok(locked.lockUntil);
    assert.equal(locked.failedLoginAttempts, 5);

    const unlocked = await repo.updateById(created.id, { lockUntil: null, failedLoginAttempts: 0 });
    assert.equal(unlocked.lockUntil, null);
...
test('incrementFailedLoginAttempts under concurrent overlapping calls: both land (no lost update)', async () => {
    ...
    const { recordset: [row] } = await db.pool.request()
        ...
        .query('SELECT failed_login_attempts FROM dbo.idp_users WHERE id = @id');
    assert.equal(row.failed_login_attempts, 3, 'all three concurrent increments must be reflected — a real UPDATE is atomic per row');
});
