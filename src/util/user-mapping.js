import { lowerGuid } from './row-mappers.js';
...
/**
 * Splits a Mongo-shaped `updateById` patch into the pieces the MSSQL adapter
 * needs to apply separately: plain `idp_users` column SETs, and the two
 * `mfaRecoveryCodes`-related side effects that target the separate
...
export function assembleUser(userRow, { externalProviders, recoveryCodes }) {
    ...
    return {
        ...
        metadata: (() => { try { return JSON.parse(userRow.metadata || '{}'); } catch { return {}; } })(),
